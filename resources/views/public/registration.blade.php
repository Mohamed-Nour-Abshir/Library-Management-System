	...
	{{-- <div class="container">
		<div class="row"> --}}
			<div class="module">
				...
				@auth('teacher')
					<form class="form-vertical" action="{{ URL::route('teacher-registration-post') }}" method="POST">
				...
				@auth('student')
					<form class="form-vertical" action="{{ URL::route('student-registration-post') }}" method="POST">
						...
						<div class="module-body">
							<div class="control-group">
								<div class="controls row-fluid">
									<input class="span6" type="text" placeholder="First Name" name="first" readonly value="{{ auth()->guard('student')->user()->first_name }}" /> 
									<input class="span6" type="text" placeholder="Last Name" name="last" readonly value="{{ auth()->guard('student')->user()->last_name }}" /> 
							...
							<div class="control-group">
								<div class="controls row-fluid">
									<input class="span4" type="email" placeholder="E-mail" name="email" autocomplete="false" readonly value="{{ auth()->guard('student')->user()->email }}" /> 
									<input class="span4" type="number" placeholder="Roll number" name="rollnumber" readonly value="{{ auth()->guard('student')->user()->roll_num }}" /> 								
									{{-- <select class="span4" style="margin-bottom: 0;" name="branch">
										<option value="0">select branch</option>
										@foreach($branch_list as $branch)
											<option value="{{ $branch->id }}">{{ $branch->branch }}</option>
									...
									<input type="hidden" name="branch" value="{{auth()->guard('student')->user()->branch }}">
									<input class="span4" type="number" placeholder="Year" name="year" value="{{auth()->guard('student')->user()->year }}" readonly /> 
									@if($errors->has('email'))
										{{ $errors->first('email')}}
									@endif
									@if($errors->has('rollnumber'))
							...
							<div class="control-group">
								<div class="controls row-fluid">
									...
									<input type="hidden" name="category" value="{{auth()->guard('student')->user()->category }}">
